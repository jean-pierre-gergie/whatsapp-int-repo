        ...
        if data:
            for document in data:
                if unique_field and collection.find_one({unique_field: document.get(unique_field)}):
                    ...
                    logger.debug(f"Inserted document into '{collection.name}' collection.")
...
def get_360_api_keys():
    """Fetch all 360 API keys from environment variables with the naming pattern `*_360_API_KEY`."""
    api_keys = {}
    for key, value in os.environ.items():
        if key.endswith('_360_API_KEY'):
            prefix = key.split('_360_API_KEY')[0].lower()  # e.g., solidarity, maronite
            api_keys[prefix] = value
    return api_keys
